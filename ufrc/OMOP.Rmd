...
str(concept)
table(concept$vocabulary_id)

# vocabulary file
vocabulary$vocabulary_id=as.factor(vocabulary$vocabulary_id)
str(vocabulary)
...
concept_vocab_merged=full_join(concept,vocabulary,by='concept_id')
...
#Filter merged data
merge_concept_filter <- merge_concept[merge_concept$concept_name != "No matching concept",]
merge_concept_filter <- merge_concept_filter[merge_concept_filter$domain_id == "Condition",]
merge_concept_filter <- merge_concept_filter[merge_concept_filter$standard_concept == "S",]
merge_concept_filter <- merge_concept_filter[merge_concept_filter$voacbulary_id == "SNOMED",]
...
#Create new columns
merge_concept_filter_new <- merge_concept_filter
temp_replace <- paste(tolower(merge_concept_filter_new$vocabulary_id),tolower(merge_concept_filter_new$concept_code),sep = ":")
merge_concept_filter_new$CONCEPT_SOURCE_CODE <- paste(merge_concept_filter_new$concept_id,temp_replace,sep = "|")
merge_concept_filter_new$CONCEPT_SOURCE_LABEL <- paste(merge_concept_filter_new$concept_id,merge_concept_filter_new$concept_name,sep="|")
merge_concept_filter_new$CONCEPT_VOCAB <- paste(merge_concept_filter_new$concept_id,merge_concept_filter_new$vocabulary_id,sep="|")
merge_concept_filter_new$CONCEPT_VOCAB_VERSION <- paste(merge_concept_filter_new$concept_id,merge_concept_filter_new$vocabulary_version,sep="|")

colnames(merge_concept_filter_new)[1,2,6] <- c("CONCEPT_ID", "CONCEPT_LABEL", "STANDARD_CONCEPT")
