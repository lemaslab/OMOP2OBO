...
condition_concept_df=full_join(concept,vocabulary,by='vocabulary_id') %>% 
  ...
  mutate(CONCEPT_SOURCE_LABEL = paste(concept_name, sep = " | ")) %>%
  mutate(CONCEPT_VOCAB = paste(tolower(vocabulary_id), sep = " | ")) %>%
  mutate(CONCEPT_VOCAB_VERSION = paste((tolower(vocabulary_version)), sep = " | ")) %>%
...
concept_ancestor_df=full_join(concept_ancestor_merged,vocabulary,by='vocabulary_id') %>%
  as_tibble() %>%
  ...
  summarize(ANCESTOR_CONCEPT_ID = str_c(concept_id, collapse = "| "),
         ANCESTOR_CONCEPT_ID_tmp=str_count(ANCESTOR_CONCEPT_ID, pattern = "|")) %>%
  filter(ANCESTOR_CONCEPT_ID_tmp==15)
         ANCESTOR_SOURCE_CODE_tmp = paste0(tolower(vocabulary_id),":",tolower(concept_code)),
         ANCESTOR_SOURCE_CODE = paste(ANCESTOR_SOURCE_CODE_tmp, sep = " | "))

# need to merge new variables into the main datframe after using summarize() funciton
