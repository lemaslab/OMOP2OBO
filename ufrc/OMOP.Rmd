...
concept_ancestor_df=full_join(concept_ancestor_merged,vocabulary,by='vocabulary_id') %>%
  ...
  group_by(concept_id) %>%
  mutate(ANCESTOR_CONCEPT_ID = str_c(descendant_concept_id, collapse = " | ")) %>%
  mutate(ANCESTOR_SOURCE_CODE = paste0(tolower(vocabulary_id),":",tolower(concept_code))) %>%
  

dim(concept_ancestor_vocab_merged)
names(concept_ancestor_vocab_merged)

head(concept_ancestor_vocab_merged)
...
# output from UFRC
save(concept_ancestor_vocab_merged, file = "concept_ancestor.RData")
  
load(file="V:/FACULTY/DJLEMAS/BENCHMARK_DATA/OMOP2OBO/cancer/data_release/concept_ancestor.RData")
